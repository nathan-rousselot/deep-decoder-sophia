...
def fit(net,
    ...
    elif OPTIMIZER == 'SophiaG':
        print("optimize with SophiaG")
        optimizer = SophiaG(p, lr=LR, weight_decay=weight_decay,rho=0.01)
    elif OPTIMIZER == 'SophiaH':
        print("optimize with SophiaH")
        optimizer = Sophia(p, lr=LR, weight_decay=weight_decay,estimator="Hutchinson")
    elif OPTIMIZER == 'OMD':
        print("optimize with Optimistic Mirror Descent")
        optimizer = OAdam(p,lr=LR)
